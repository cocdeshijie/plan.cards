...
export function PortfolioWidget({ className }: { className?: string }) {
  ...
  const stats = useMemo(() => {
    ...
    const totalFees = active.reduce((sum, card) => {
    ...
    return { active: active.length, closed: closed.length, totalFees, issuerBreakdown };
  ...
  const statCards = [
    ...
    { label: "Annual Fees (Net)", value: `$${stats.totalFees.toLocaleString()}`, icon: DollarSign, color: "text-orange-500" },
  ];
